...
if (Test-Path $InstallDir) {
  # ── Stop any running Krythor gateway before attempting the rename ───────────
  # Windows locks files held open by a running process — the rename will fail
  # with "Access denied" if the gateway is still up.  Try a graceful API stop
  # first (works when gateway is healthy), then fall back to killing the node
  # process that has the install directory in its command line.
  Write-Step "Checking for running Krythor process..."

  $GatewayPort = if ($env:KRYTHOR_GATEWAY_PORT) { $env:KRYTHOR_GATEWAY_PORT } else { '47200' }

  # 1. Graceful stop via API (best effort — ignore errors)
  try {
    $stopHeaders = @{ 'Content-Type' = 'application/json' }
    $tokenFile   = Join-Path $InstallDir 'config\gateway-token.txt'
    if (Test-Path $tokenFile) {
      $tok = (Get-Content $tokenFile -Raw).Trim()
      if ($tok) { $stopHeaders['Authorization'] = "Bearer $tok" }
    }
    Invoke-RestMethod -Uri "http://localhost:$GatewayPort/api/shutdown" `
      -Method POST -Headers $stopHeaders -TimeoutSec 4 -ErrorAction SilentlyContinue | Out-Null
    Start-Sleep -Seconds 2
    Write-Ok "Sent shutdown signal to gateway"
  } catch { <# gateway may not be running — ignore #> }

  # 2. Kill any node.exe process whose command line references the install dir
  #    (catches the runtime node.exe bundled inside ~/.krythor/runtime/)
  ...
  foreach ($proc in $procs) {
    ...
    if ($cmdLine -like "*$InstallDirNorm*") {
      try {
        ...
        Write-Ok "Stopped Krythor process (PID $($proc.ProcessId))"
      } catch { <# already gone #> }
    }
  }
  ...
  try {
    ...
    Write-Fail "Could not move old install directory. The Krythor gateway may still be running. Close it and try again. Error: $_"
  }
}
...
if "%1"=="update" (
  ...
  echo Stopping Krythor gateway...
  :: 1. Graceful API shutdown (works when gateway is healthy)
  set _PORT=47200
  if not "%KRYTHOR_GATEWAY_PORT%"=="" set _PORT=%KRYTHOR_GATEWAY_PORT%
  curl -sf -X POST http://localhost:%_PORT%/api/shutdown -H "Content-Type: application/json" >nul 2>&1
