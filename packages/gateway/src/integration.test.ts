...
describe('Integration — applySchema (migration + integrity check)', () => {
  it('runs all 10 migrations on a fresh in-memory DB and reports ok integrity', () => {
    const db = new Database(':memory:');
    const result = applySchema(db);

    expect(result.migration.applied).toBe(10);
    expect(result.migration.total).toBe(10);
    expect(result.migration.userVersion).toBe(10);
  ...
  it('applySchema is idempotent — second call applies 0 migrations', () => {
    ...
    expect(second.migration.userVersion).toBe(10);
