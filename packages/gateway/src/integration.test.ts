...
describe('Integration — applySchema (migration + integrity check)', () => {
  it('runs all 9 migrations on a fresh in-memory DB and reports ok integrity', () => {
    const db = new Database(':memory:');
    const result = applySchema(db);

    expect(result.migration.applied).toBe(9);
    expect(result.migration.total).toBe(9);
    expect(result.migration.userVersion).toBe(9);
  ...
  it('applySchema is idempotent — second call applies 0 migrations', () => {
    ...
    expect(second.migration.userVersion).toBe(9);
