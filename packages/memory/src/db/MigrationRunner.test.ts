...
describe('MigrationRunner — user_version', () => {
  ...
  it('run() returns correct total count from real migration files', () => {
    ...
    // We have 7 migration files (001–007)
    expect(result.total).toBe(7);
    expect(result.applied).toBe(7);
    expect(result.userVersion).toBe(7);
    expect(runner.getUserVersion()).toBe(7);
    db.close();
  });
  ...
  it('getAppliedVersions returns all versions after a fresh run', () => {
    ...
    const runner = new MigrationRunner(db);
    runner.run(dbPath);
    expect(runner.getAppliedVersions().size).toBe(7);
    db.close();
  });
  ...
  it('user_version matches applied migration count after run', () => {
    ...
    expect(result.userVersion).toBe(result.total); // all 7 applied → version 7
    db.close();
  });
});
