...
describe('MigrationRunner — user_version', () => {
  ...
  it('run() returns correct total count from real migration files', () => {
    ...
    // We have 16 migration files (001–016)
    expect(result.total).toBe(16);
    expect(result.applied).toBe(16);
    expect(result.userVersion).toBe(16);
    expect(runner.getUserVersion()).toBe(16);
    db.close();
  });
  ...
  it('getAppliedVersions returns all versions after a fresh run', () => {
    ...
    const runner = new MigrationRunner(db);
    runner.run(dbPath);
    expect(runner.getAppliedVersions().size).toBe(16);
    db.close();
  });
  ...
  it('run() is idempotent — second call applies 0 new migrations', () => {
    ...
    expect(second.userVersion).toBe(16);
    db.close();
  });
  ...
  it('user_version matches applied migration count after run', () => {
    ...
    expect(result.userVersion).toBe(result.total); // all 12 applied → version 12
    db.close();
  });
});
