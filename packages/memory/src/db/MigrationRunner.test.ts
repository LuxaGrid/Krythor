...
describe('MigrationRunner — user_version', () => {
  ...
  it('run() returns correct total count from real migration files', () => {
    ...
    // We have 15 migration files (001–015)
    expect(result.total).toBe(15);
    expect(result.applied).toBe(15);
    expect(result.userVersion).toBe(15);
    expect(runner.getUserVersion()).toBe(15);
    db.close();
  });
  ...
  it('getAppliedVersions returns all versions after a fresh run', () => {
    ...
    const runner = new MigrationRunner(db);
    runner.run(dbPath);
    expect(runner.getAppliedVersions().size).toBe(15);
    db.close();
  });
  ...
  it('run() is idempotent — second call applies 0 new migrations', () => {
    ...
    expect(second.userVersion).toBe(15);
    db.close();
  });
