...
describe('MigrationRunner — user_version', () => {
  ...
  it('run() returns correct total count from real migration files', () => {
    ...
    // We have 9 migration files (001–009)
    expect(result.total).toBe(9);
    expect(result.applied).toBe(9);
    expect(result.userVersion).toBe(9);
    expect(runner.getUserVersion()).toBe(9);
    db.close();
  });
  ...
  it('getAppliedVersions returns all versions after a fresh run', () => {
    ...
    const runner = new MigrationRunner(db);
    runner.run(dbPath);
    expect(runner.getAppliedVersions().size).toBe(9);
    db.close();
  });
  ...
  it('run() is idempotent — second call applies 0 new migrations', () => {
    ...
    expect(second.userVersion).toBe(9);
    db.close();
  });
  ...
  it('user_version matches applied migration count after run', () => {
    ...
    expect(result.userVersion).toBe(result.total); // all 9 applied → version 9
    db.close();
  });
});
